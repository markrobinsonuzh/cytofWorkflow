...
```{r load-metadata}
data("PBMC_md", package = "CATALYST")
md <- PBMC_md
head(md)
```

The `sample_id` variable will be used all over the workflow to label the samples. It is important to assure that variables are of the desired type (factor, numeric, character), since input methods may convert columns into different data types. For the statistical modeling, we want to make the condition variable a factor with the reference (`Ref`) samples being the reference level, where the order of factor levels can be defined with the `levels` parameter of the `factor` function.
...
In our example, information about the panel is also available through `r Biocpkg "CATALYST"`. We again loaded via the `data` function and store it into a variable called `panel`.  It contains a column for the `fcs_colname` that corresponds to the original measurement parameter in the raw data, and `antigen`, which specifies the protein marker that was targeted. Two additional columns specify whether a channel captures a type (lineage) or state (functional) type of marker. It should be made sure that elements from `panel$fcs_colname` can be matched with the column names of the input `flowSet`. 

```{r load-panel}
data("PBMC_panel", package = "CATALYST")
panel <- PBMC_panel
head(panel)
all(panel$fcs_colname %in% colnames(fcs_raw))
```

The isotope, metal and antigen information that the instrument receives is stored in the `flowFrame` (container for one sample) or `flowSet` (container for multiple samples) objects. You can type `fcs_raw[[1]]` to see the first `flowFrame`, which contains a table with columns `name` and `desc`. Their content can be accessed with functions `pData(parameters())`, which is identical for all the `flowFrame` objects in the `flowSet`. The variable `name` corresponds to the column names in the `flowSet` object, you can type in R `colnames(fcs_raw)`.
...
As the ranges of marker intensities can vary substantially, we give the option to scale expressions of all markers to values between 0 and 1 using low (e.g. 1\%) and high (e.g. 99\%) percentiles as the boundary. This additional transformation of the arcsinh-transformed data can sometimes give better representation of relative differences in marker expression between annotated cell populations, however, it is used here for visualization only.

# Data organization

Throughout this workflow, we will store measurement data, metadata, and panel in an object of class `daFrame`. To simplify downstream analysis, the `daFrame` constructor will

* replace all dashes with underscores in both channel and marker names to avoid problems in matching upon object conversion from one type to another (e.g. from matrix to data.frame) that cause some characters (e.g. dashes) in the dimension names to be replaced with dots,
* use the marker IDs as specified in the `panel$antigen` column rather than channel names for both data accession and visualization, and
* perform an arcsinh transformation of the data with a default cofactor 5.
...
df <- data.frame(panel, lineage=as.numeric(panel$antigen %in% lineage_markers), functional=as.numeric(panel$antigen %in% functional_markers))
lineage_markers <- 

# Lineage markers
(lineage_markers <- panel$antigen[panel$type_marker == 1])

# Functional markers
(functional_markers <- panel$antigen[panel$state_marker == 1])

fcs_colnames <- colnames(fcs_raw)[panel_fcs$des %in% c(lineage_markers, functional_markers)]
WriteXLS::WriteXLS(df, "/users/hlc/desktop/PBMC8_panel2.xlsx")
...
The step below generates a plot with per-sample marker expression distributions, colored by condition (see Figure \@ref(fig:plot-marker-expression-distribution)). Here, we can already see distinguishing markers, such as pNFkB and CD20, between stimulated and unstimulated conditions. 
...
plotCounts(data, color_by = "condition")
```
...
plotMDS(data, color_by = "condition")
```
...
plotExprHeatmap(data, anno = TRUE, color_by = "condition", palette = brewer.pal(n = 8, name = "YlGnBu"), 
    scale = TRUE, clustering_distance = "euclidean", clustering_linkage = "average")
```
...
plotNRS(data, color_by = "condition")
```
...
```{r flowsom-som}
lineage_markers <- panel$antigen[panel$type_marker == 1]
data <- cluster(data, cols_to_use = lineage_markers, xdim = 10, ydim = 10, maxK = 20)
```
...
plotClusterHeatmap(data, hm2 = "pS6")
```
...
## run t-SNE: Subsample n cells per sample
data <- tSNE(data, n = 2000)
```
...
## Facet per condition
plotSNE(data, color_by = 20, facet = "condition")
```
...
plotClusterHeatmap(data, hm2 = "pS6", k = 100, m = 20, cluster_anno = FALSE)
```
...
```{r cluster-merging1}
data("merging_table", package = "CATALYST")
merging_table

data <- mergeClusters(data, table = merging_table, id = "merging1")
```
...
plotSNE(data, color_by = "merging1")
```
...
plotClusterHeatmap(data, k = 20, m = "merging1")
```
...
plotClusterHeatmap(data, k = "merging1")
```
...
data <- mergeClusters(data, table = cluster_merging2, id = "merging2")
```
...
plotSNE(data, color_by = "merging2")
```
...
plotClusterHeatmap(data, k = 20, m = "merging2")
```
...
plotClusterHeatmap(data, k = "merging2")
```
...
# tabular comparison of # metaclusters = 8 and 2nd manual merging
codes <- metadata(data)$cluster_codes
table(algorithm=codes$`8`, manual=codes$`merging2`)
```
...
design <- createDesignMatrix(metadata(data)$experiment_info, cols_design=3:4)
...
```{r diff-freqs}
res_DA <- diffcyt(data, design, contrast, analysis_type = "DA", method_DA = "diffcyt-DA-edgeR")
```
...
plotAbundances(data, k = "merging2", by = "sample_id", facet = "condition")
```
...
plotAbundances(data, k = "merging2", by = "cluster_id", facet = "condition")
```
...
formula_glmer_binomial1 <- createFormula(metadata(data)$experiment_info, cols_fixed = 3, cols_random = 2)
formula_glmer_binomial2 <- createFormula(metadata(data)$experiment_info, cols_fixed = 3, cols_random = c(2, 4))
```
...
counts <- calcCounts(data, )
res_DA_1 <- testDA_GLMM(counts, formula_glmer_binomial1, contrast, )
res_DA_2 <- testDA_GLMM(counts, formula_glmer_binomial2, contrast)

contrast <- createContrast(c(0, 1))
diffcyt(data, design, contrast,
...
table(rowData(res_DA_1)$p_adj < FDR_cutoff)
table(rowData(res_DA_2)$p_adj < FDR_cutoff)
...
plotDiffHeatmap(data, res_DA_1, th = FDR_cutoff)
...
plotMedExprs(data, k = "merging1", facet = "cluster_id", group_by = "condition")
