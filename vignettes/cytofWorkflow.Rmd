...
We use a subset of CyTOF data originating from Bodenmiller *et al.* [@Bodenmiller2012] that was also used in the `r Githubpkg("nolanlab/citrus")` paper [@Bruggner2014]. 
...
It is important to point out that we cluster all cells from all samples together. This strategy is beneficial, since [+ we directly obtain cluster assignmnet for each cell,] we label cell populations only once and the mapping of cell types between samples is automatically consistent. [+ For a list of alternative appraoches its advantages and disadvantages, please reffer to the Discussion section, where we discuss, for instance, clustering per sample, clustering of data from different measurement batches, cell down-sampling in case of unequal number of cells in samples.]
...
[+ When the interst is in studying more specific subpopulations, one can follow a strategy of reclustering described in the Obtaining higher resolution section, where we propose to repeat the workflow (clustering and differential analyses) on data restricted only to a selected cell type for which more resolution is desired. ]
...
plot_clustering_heatmap_wrapper2 <- function(expr, expr01,
  ...
  ### Annotation of the original clusters
  ...
  ### Annotation of the merged clusters
  if(!is.null(cluster_merging)){
  ...
  if(!is.null(cluster_merging)){
    ha2 <- Heatmap(annotation_row2, name = "Cluster merging", 
  ...
  ha_text <- rowAnnotation(text = row_anno_text(labels_row, 
    gp = gpar(fontsize = 8)), width = max_text_width(labels_row))
  ...
  ht1 <- Heatmap(expr_heat, name = "Normalized \nmedian expression", 
    column_title = "Lineage markers", col = color_heat, 
  ...
  if(!is.null(functional_markers)){
    for(i in 1:length(functional_markers)){
      ...
      functional_marker <- functional_markers[i]
      
      ## Rearange so the rows represent clusters and markers
      expr_heat_fun <- as.matrix(dcast(expr_median_sample_cluster_tbl[, 
        c("sample_id", "cluster", functional_marker)], 
        cluster ~ sample_id, value.var = functional_marker)[, -1])
      
      draw_out <- draw_out + Heatmap(expr_heat_fun, 
        column_title = functional_marker, col = color_heat, 
        cluster_columns = FALSE, cluster_rows = cluster_rows, 
...
[+ Using heatmaps, we can also visualize median marker expression in the 100 SOM codes as in Figure \@ref(fig:plot-som-codes1-heatmap-complex). Of note, the clustering presented with the dendrogram does not completely agree with the clustering depicted by the 20 colors becasue the first one is based on the hierarchical clustering with average linkage and Euclidean distance, while the second one results from the consensus clustering.]


```{r plot-som-codes1-heatmap-complex, fig.height = 12, fig.cap = "Median marker expression in the 100 SOM codes."}
plot_clustering_heatmap_wrapper2(expr = expr, expr01 = expr01,
  lineage_markers = lineage_markers, functional_markers = "pS6", 
