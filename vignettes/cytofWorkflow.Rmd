...
To be able to analyze arbitrary experimental designs (e.g., batch effects, paired experiments, etc.), we show how to conduct differential analysis of cell population abundances using generalized linear mixed models (GLMM) and of marker intensities using linear models (LM) and linear mixed models (LMM).  For both differential abundance and expression analysis, we use methods implemented in the `r Biocpkg("diffcyt")` package [@Weber2018].  Internally, model fitting is performed with packages `r CRANpkg("lme4")` and `stats`, and hypothesis testing with the `r CRANpkg("multcomp")` package.
...
In our example, the .fcs files listed in the metadata can be loaded from the `r Biocpkg("HDCytoData")` package [@Weber2019].
...
`r Biocpkg("CATALYST")` provides a flexible wrapper, `runDR`, to perform the set of dimensionality reduction methods availible in the `r Biocpkg("scater")` package. To make results reproducible, the random seed should be set via `set.seed` *prior* to calling `runDR`. The subset of markers to use for computing reduced dimensions is specified via `cols_to_use`, but will default to the set of type-markers defined in input `daFrame` if unspecified (`type_markers(daf)`); the cells to use are specified with `rows_to_use`. When a single numeric value N is provided, `runDR` will draw a random subset of N cells per sample.
...
```{r umap-one-clustering1, fig.cap = "t-SNE and UMAP based on the arcsinh-transformed expression of the 10 lineage markers in the cells from the PBMC dataset. From each of the 16 samples, 500 (t-SNE) and 2000 (UMAP) cells were randomly selected. Cells are colored according to the 20 cell populations obtained with FlowSOM after the metaclustering step with ConsensusClusterPlus."}
...
We use the `r Biocpkg("diffcyt")` package [@Weber2018] to perform the differential analyses. The `r Biocpkg("diffcyt")` package includes implementations of several different methods for differential testing; including methods based on mixed models, which we use here. The mixed models methodology uses the `stats` and `r CRANpkg("lme4")` packages to fit the fixed and mixed models, respectively, and the `r CRANpkg("multcomp")` package for hypothesis testing.
...
We set up the model formulas and contrast matrix using the `createFormula()` and `createContrast()` functions from the `r Biocpkg("diffcyt")` package. These functions create the model formulas and contrast matrix in the required format for the `r Biocpkg("diffcyt")` testing functions. For more details, see `?createFormula` and `?createContrast`, or refer to the extended documentation in the `r Biocpkg("diffcyt")` Bioconductor vignette ([diffcyt workflow](http://bioconductor.org/packages/release/bioc/vignettes/diffcyt/inst/doc/diffcyt_workflow.html)).

```{r da-formulas}
ei <- metadata(daf)$experiment_info
(da_formula1 <- createFormula(ei, cols_fixed = "condition", 
                              cols_random = "sample_id"))
(da_formula2 <- createFormula(ei, cols_fixed = "condition",
...
There might be instances of small cell populations for which no cells are observed in some samples or where the number of cells is very low.  For clusters absent from a sample (e.g., due to biological variance or insufficient sampling), NAs are introduced because no median expression can be calculated; in the case of few cells, the median may be quite variable.  Thus, we apply a filter to remove samples with very low counts (by default, clusters are kept if they have at least 3 cells in at least half the total number of samples, which is appropriate for a two-group comparison with equal numbers of samples per condition).
...
```{r ds-analysis}
ds_res1 <- diffcyt(daf, formula = ds_formula1, contrast = contrast,
    analysis_type = "DS", method_DS = "diffcyt-DS-LMM",
    clustering_to_use = "merging1", verbose = FALSE)
table(rowData(ds_res1$res)$p_adj < FDR_cutoff)
ds_res2 <- diffcyt(daf, formula = ds_formula2, contrast = contrast,
    analysis_type = "DS", method_DS = "diffcyt-DS-LMM",
    clustering_to_use = "merging1", verbose = FALSE)
table(rowData(ds_res2$res)$p_adj < FDR_cutoff)
```

A summary table displaying the results (raw and adjusted p-values) together with the observed median marker expression in each cluster and sample can be generated using `r Biocpkg("diffcyt")`'s `topTable()` function.
...
Similar to the analysis above, we identify more markers being differentially expressed with the linear mixed model, which accounts for the within patient variability.
...
As before, we create a summary table displaying the results (raw and adjusted p-values) together with the observed median marker expression in each sample, and plot a heatmap with the markers sorted according to significance (Figure \@ref(fig:diff-expr1-plot-heatmap-with-significant-markers)).
...
We note that the LM, LMM and GLMM may perform poorly for extremely small samples.  Here, solutions similar to those widely accepted in transcriptomics that share information over variance parameters [@Robinson2007; @Love2014; @Ritchie2015] can instead be leveraged.  We have recently implemented methods based on these ideas in the `r Biocpkg("diffcyt")` package [@Weber2018].  Similarly, the `r Biocpkg("cydar")` [@Lun2017] package performs differential abundance analysis (on "hypersphere" counts) using the generalized linear modeling capabilities of `r Biocpkg("edgeR")` [@McCarthy2012].  For a detailed explanation and comparison of these alternative approaches, see @Weber2018.
...
MN and MDR designed methodology and designed and ran analyses. MN drafted the manuscript with input from MDR. HC implemented code and contributed to drafting. CK and SG performed experiments, analyzed data and gave feedback on clinical applications. CK performed the cluster merging. LMW contributed code and ideas for the analyses, and contributed to drafting. FJH contributed code and ideas for the analyses. MPL interpreted data and provided clinical perspective. BB gave feedback on the manuscript and the bioinformatics. All authors read and approved the final manuscript and have agreed to the content.
...
MN acknowledges the funding from a Swiss Institute of Bioinformatics (SIB) Fellowship. LMW was supported by a Forschungskredit (Candoc) grant from the University of Zurich (FK-17-100).
