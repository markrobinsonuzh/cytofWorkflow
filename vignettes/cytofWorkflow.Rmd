...
In this workflow, we have presented a pipeline for CyTOF data analysis that consists of a quality control step where aggregate characteristics of the samples are visualized, for example with an MDS plot, allowing verification of the experimental design, detection of batch effects and outlying samples.Next, cell population identification was carried out, forming the basis for subsequent differential analyses of cell population abundance, differential marker expression within a population or overall marker expression differences.
The approaches to differential analyses proposed here are able to model complex designs via design matrices, such as factorial experiments, paired experiments or adjustment for batch effects. We have presented a range of visualizations that help in understanding the data and reporting the results of clustering and differential analyses.
...
In particular, getting the right resolution of clusters is critical since there can be situations where a biologically meaningful cell population may be differentially enriched between conditions, but was notbe detected because it was combined with another cell population that behaves different. 
...
In case of batches, for example, samples were measured with two different runs of CyTOF, and clear differences between these time points can be observed in the spot check plots (Section [spot check plots](#diagnostic-plots)), one could consider separate clustering of samples from these batches.
Clustering of them altogether may result in an inaccurate cell assignment to different populations and might lead to artifacts in the following analysis, such as differential abundance analysis.
...
We have shown how over-clustering is convenient for detecting meaningful cell populations. However, too high resolution may complicate manual cluster merging, as profiles of some of the clusters may not represent the canonical cell populations when looking only at the heatmap of median marker expression in each cluster. 
