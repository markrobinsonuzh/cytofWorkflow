...
Mass (and high dimensional flow) cytometry experiments have become a method of choice for characterizing cell populations at the single cell level at the high-throughput. Here, we present a pipeline of analyses based on Bioconductor packages, identifying cell populations via clustering using `FlowSOM` combined with manual merging and exploring the differences in cell composition across samples with the differential abundance and differential marker expression analyses. Differential analyses are based on regression frameworks and thus able to model a wide range of experimental designs, such as those containing batches.  Application of generalized linear mixed models for differential cell population abundance overdispersion of cell counts in clusters to be considered. We include various visualizations for quality control (multi-dimensional scaling  plots), for reporting of clustering results (t-SNE dimension reduction, heatmaps with dendrograms) and for differential analyses.
...
CyTOF utilizes transition element isotopes to label antibodies and abundances per cell are recorded with a time-of-flight mass spectrometer. 
