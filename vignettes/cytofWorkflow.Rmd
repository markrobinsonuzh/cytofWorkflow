...
We use the `r Biocpkg("diffcyt")` package [@Weber2018] to perform the differential analyses, which includes implementations of various  methods for differential testing, such as linear mixed models. The mixed models methodology uses the `stats` and `r CRANpkg("lme4")` packages to fit the fixed and mixed models, respectively, and the `r CRANpkg("multcomp")` package for hypothesis testing.
