...
Most DR techniques generally require significant computational time to process the data. To keep running times reasonable for larger CyTOF datasets, one may use a subset of cells. DR with `r Biocpkg("CATALYST")` has been implemented in a way that allows using different sets of cells for different DRs. For example, here we use 500 and 1000 cells from each sample to run t-SNE and UMAP, respectively.
...
```{r ds-formula}
formula_lm <- createFormula(ei, cols_fixed = "condition")
formula_lmer <- createFormula(ei, cols_fixed = "condition", cols_random = "sample_id")
```
