...
[NEW: In the following analysis, often we will use marker IDs as column names in the tables containing expression values. During object conversion from one type to another, for example, from matrix to data frame, characters, such as dashes, in dimension names are, by default, replaced with dots. This may be problematic, as the new names can not be matched. To avoid this problem, we replace all the dashes with underscores.]
...
```{r plot_merker_expression_distribution, fig.cap = "As a diagnostic, per-sample marker expression distributions are shown. Samples are colored according to experimental condition."}
library(ggplot2)
library(reshape2)
...
[NEW: For this experiment, it may be interesting to additionally depict the patient information. We do it by plotting a different point shape for each patient. Indeed, we can see that often the direction of abundance changes between the two conditions is concordant between the patients.]


```{r diff_freqs_plot_props_boxplot, fig.width = 8, fig.height = 4, fig.cap="Relative abundance of PBMC populations in each sample plotted using boxplots."}
...
da_out <- differential_abundance_wrapper(counts, md = md, 
  formula = formula_glmer_binomial, K = K)
apply(da_out$adjp < 0.05, 2, table)
```


[NEW: Just testing this model with patient and sample as random effects.]

```{r diff_freqs_fit_model2}
da_out <- differential_abundance_wrapper(counts, md = md, 
  formula = y/total ~ condition + (1|patient_id) + (1|sample_id), K = K)
da_out$adjp < 0.05
```


An output table containing the observed cell population proportions in each sample and p-values can be assembled. 

```{r diff_freqs_fit_model_output}
da_output <- data.frame(cluster = rownames(props), props, da_out$pvals, da_out$adjp)
```
...
sign_clusters <- names(which(sort(da_out$adjp[, "adjp_BCRXLvsRef"]) < FDR_cutoff))
## Get the adjusted p-values
sign_adjp <- da_out$adjp[sign_clusters , "adjp_BCRXLvsRef", drop=FALSE]
...
```{r diff_expr2_plot_median_expr, fig.width = 8, fig.height = 9, fig.cap = "Median expression of 14 signaling markers in the identified PBMC cell populations."}
...
```{r diff_expr2_fit_model_output}
de_output2 <- data.frame(expr_median_sample_cluster, de_out2$pvals, de_out2$adjp)
```
...
```{r diff_expr1_plot_median_expr, fig.width = 8, fig.height = 7, fig.cap="Median expression of 14 signaling markers calculated from all the cells in a given sample."}
