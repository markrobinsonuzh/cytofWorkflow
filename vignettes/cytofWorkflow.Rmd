...
Standard flow cytometry experiments measure 6-12 parameters where the most modern flow cytometers can measure up to 20 parameters [Mahnke, Roederer Clin Lab Med 2007] and new developments (e.g., BD FACSymphony) might increase this capacity towards 50 markers.
...
The preprocessing steps including normalization using bead standards, debarcoding and compensation can be completed with the `r Biocpkg("CATALYST")` package, which provides an implementation of the debarcoding algorithm described by Zunder *et al.* [@Zunder2015] and the bead-based normalization from Finck *et al.* [@Finck2013].  Alternatively, datasets can be preprocessed outside of R (e.g., Normalizer v0.3 [https://github.com/nolanlab/bead-normalization/releases/tag/v0.3]).
...
Instead of t-SNE, one could also use other dimension reduction techniques, such as PCA, diffusion maps, SIMLR [REF] or isomaps, some of which are conveniently available via the `cytof_dimReduction` function from the `r Biocpkg("cytofkit")` package [REF]. 
