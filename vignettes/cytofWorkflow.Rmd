...
```{r plot-merker-expression-distribution, fig.cap = "Diagnostic, per-sample marker expression distributions are shown. Samples are colored according to experimental condition."}
library(ggplot2)
library(reshape2)
...
Another spot check is the number of cells per sample.  This plot can be used as a guide together with other readouts to identify samples where not enough cells were assayed.
<!-- [What is the minimum amount of cells recommended for analysis???? What is the maximum???] -->


```{r plot-number-of-cells, fig.cap = "Barplot with the number of cells per sample."}
...
```{r plot-mds, fig.cap = "MDS plot based on the median marker expression."}
...
```{r plot-dendogram, fig.cap = "Clustering of samples and markers based on the median marker expression."}
...
```{r nrs, fig.cap = "Non-redundancy scores. The colored points represent the per-sample NR scores, while open white circles indicate the mean NR scores from all the samples. Markers on the x-axis are sorted according to the decreasing average NRS."}
...
Instead of using only medians, which do not give a full representation of cluster specifics, one can plot the entire marker expression distribution in each cluster.  Such a plot gives more detailed profile of each cluster, but represents an increase in the amount of information to interpret.  Heatmaps give the overall overview of clusters, are quicker and easier to interpret, and together with the dendrogram can be a good basis for further cluster merging (see [Cluster merging and annotation](#cluster-merging-and-annotation) section).
...
```{r plot-clustering-heatmap1, fig.cap = "Heatmap of the median marker intensities in the 20 cell populations obtained from the metaclustering step. The dendrogram on the left represents the hierarchical similarity between the metaclusters."}
...
```{r plot-clustering-distribution1, fig.height = 8, fig.cap = "Distributions of marker intensities in the 20 cell populations obtained from the metaclustering step. Blue densities represent marker expression for cells in given clusters. Grey densities are calculated from all the cells and serve as a reference."}
...
```{r tsne-plot-one-expr-CD4, fig.cap = "t-SNE plot with cells colored according to the expression level of the CD4 marker."}
...
[CHANGED!] When the figure is stratified by sample, we can verify whether similar cell populations are present in replicates, which can help in identifying outlying samples if any are present. Optionally, stratification can be done by condition. With such a spot-check plot, we can inspect whether differences in cell abundance are strong between conditions, and we can identify distinguishing clusters. 


```{r tsne-plot-one-clustering1, fig.cap = "t-SNE plot with cells colored according to the 20 metaclusters."}
...
```{r tsne-plot-facet-sample, fig.cap = "t-SNE plot stratified by sample."}
## Facet per sample
ggp + facet_wrap(~ sample_id) 
```


```{r tsne-plot-facet-condition, fig.height = 4, fig.cap = "t-SNE plot stratified by condition."}
## Facet per condition
...
```{r plot-som-codes1-pca, fig.height = 8, fig.cap = "(A) t-SNE plot and (B) PCA plot representing the 100 SOM codes colored according to the metaclustering into 20 cell populations. The SOM codes represent characteristics of the 100 (by default) clusters generated in the first step of the FlowSOM pipeline."}
## Plot PCA on codes
...
```{r tsne-plot-one-clustering1m, fig.cap = "t-SNE plot with cells colored according to the merging of 20 metaclusters."}
...
```{r plot-clustering-heatmap1m-merging, fig.cap='Heatmap showing how the 20 metaclusters are merged.'}
plot_clustering_heatmap_wrapper(expr = expr[, lineage_markers_ord],
...
```{r plot-clustering-heatmap1m, fig.height = 3, fig.cap='Heatmap of cell populations obtained from merging the 20 metaclusters.'}
plot_clustering_heatmap_wrapper(expr = expr[, lineage_markers_ord],
...
![The delta area plot from ConsensusClusterPlus indicating the relative increase in cluster stability obtained when clustering into k groups.](consensus_plots/consensus022.png){width=70%}
...
```{r tsne-plot-one-clustering2, fig.cap = "t-SNE plot with cells colored according to the 12 metaclusters."}
...
```{r plot-clustering-heatmap2, fig.height = 3, fig.cap = "Heatmap of the median marker intensities in 12 cell populations obtained from the metaclustering step."}
plot_clustering_heatmap_wrapper(expr = expr[, lineage_markers_ord],
  expr01 = expr01[, lineage_markers_ord], cell_clustering = cell_clustering2,
  color_clusters = color_clusters)
```

Over-clustering into as few as 12 groups still allows us to identify the same 8 cell populations as when merging 20 clusters, but it is simpler to define since fewer profiles need to be manually scanned.  The expert-based merging is saved in the PBMC8_cluster_merging2.xlsx file on the [Robinson Lab server](http://imlspenticton.uzh.ch/robinson_lab/cytofWorkflow).
...
```{r tsne-plot-one-clustering2m, fig.cap = "t-SNE plot with cells colored according to the merging of 12 metaclusters."}
...
```{r plot-clustering-heatmap2-merging, fig.cap = "Heatmap of the median marker intensities in 12 cell populations obtained from the metaclustering step along with the annotation of cell populations."}
...
```{r plot-clustering-heatmap2m, fig.height = 3, fig.cap='Heatmap of cell populations obtained from merging the 12 metaclusters.'}
plot_clustering_heatmap_wrapper(expr = expr[, lineage_markers_ord],
...
In the t-SNE map, we can see that part of the new cell populations (cluster 7, 1 and 4, 2, 5 and 8) overlap substantially with populations obtained by the means of manual merging (CD4 T-cells, B-cells, surface-, monocytes and DC).  However, cells that belong to clusters 3 and 6 are subdivided in a different manner according to the manual merging.  Cluster 3 consists of CD8 T-cells and NK cells, and the latter cannot be identified anymore based on the heatmap corresponding to clustering into 8 groups. 

```{r tsne-plot-one-clustering3, fig.height = 8, fig.cap = "t-SNE plot with cells colored according to (A) the expert merging of 12 metaclusters into 8 cell populations; and (B) the 8 automatically detected metaclusters."}
...
```{r plot-clustering-heatmap3, fig.height = 3, fig.cap = "Heatmap of the median marker intensities in 8 cell populations obtained from the metaclustering step."}
plot_clustering_heatmap_wrapper(expr = expr[, lineage_markers_ord],
...
For each sample, we plot its PBMC cell type composition represented with colored bars, where the size of a given stripe reflects the proportion of the corresponding cell type in a given sample.

```{r diff-freqs-plot-props-barplot, fig.cap="Relative abundance of PBMC populations in each sample, represented with a barplot."}
...
```{r diff-freqs-plot-props-boxplot, fig.height = 4, fig.cap="Relative abundance of PBMC populations in each sample plotted using boxplots."}
...
```{r diff-freqs-plot-heatmap-with-significant-clusters, fig.height = 3, fig.cap="Normalized proportions of PBMC cell populations that are significantly differentially abundant between BCR/FcR-XL stimulated and unstimulated condition."}
...
```{r diff-expr2-plot-median-expr, fig.height = 8, fig.cap = "Median expression of 14 signaling markers in the identified PBMC cell populations."}
...
```{r diff-expr2-plot-heatmap-with-significant-markers, fig.height = 10, fig.cap="Normalized expression of signaling markers that are significantly differentially expressed between BCR/FcR-XL stimulated and unstimulated condition."}
...
```{r diff-expr1-plot-median-expr, fig.height = 6, fig.cap="Median expression of 14 signaling markers calculated from all the cells in a given sample."}
...
```{r diff-expr1-plot-heatmap-with-significant-markers, fig.height = 3, fig.cap="Normalized expression for signaling markers that are significantly differentially expressed between BCR/FcR-XL stimulated and unstimulated condition."}
