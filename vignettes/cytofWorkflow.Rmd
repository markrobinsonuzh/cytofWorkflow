...
<!-- However, this approach works only if the variability between samples is small enough. -->
<!-- In case of batches, for example, samples were measured with two different runs of CyTOF and clear differences between these time points can be observed in the spot-check plots (Section [Spot-check plots](#spot-check-plots)), one should consider separate clustering of samples from these batches. -->
<!-- Clustering them together may result in an inaccurate cell assignment to different populations and might lead to artifacts in the following analysis, such as differential abundance analysis. -->
<!-- However, to be able to use results from separate clustering analysis, for example, in the differential analysis, one has to make sure that the mapping of cell populations between them is accurate by, for example, using the same cluster names. -->
