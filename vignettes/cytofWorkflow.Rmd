...
# 7
fig_nums(name = "plot-clustering-distribution1", caption = "Distributions of marker intensities (arcsinh-transformed) of the 10 lineage markers in the 20 cell populations obtained with FlowSOM after the metaclustering step with ConsensusClusterPlus (PBMC data). Red densities represent marker expression for cells in the given cluster. Green densities are calculated from all the cells and serve as a reference.")
# 8
...
# 10
fig_nums(name = "tsne-plot-facet-sample", caption = paste0("t-SNE plot as in the Figure ", ifelse(on.bioc, fig_nums("tsne-plot-one-clustering1", display = "num"), "\\@ref(fig:tsne-plot-one-clustering1)"), ", but stratified by sample."))
# 11
fig_nums(name = "tsne-plot-facet-condition", caption = paste0("t-SNE plot as in the Figure ", ifelse(on.bioc, fig_nums("tsne-plot-one-clustering1", display = "num"), "\\@ref(fig:tsne-plot-one-clustering1)"), ", but stratified by condition."))
# 12
fig_nums(name = "plot-som-codes1-pca", caption = "(A) t-SNE plot and (B) PCA plot representing the 100 SOM codes in the PBMC dataset colored according to the metaclustering with ConsensusClusterPlus into 20 cell populations. The SOM codes represent characteristics of the 100 (by default) clusters generated in the first step of the FlowSOM pipeline. The size of the points corresponds to the number of cells that were assigned to a given code.")
# 13
...
# 16
fig_nums(name = "consensus-plot", caption = "The delta area plot generated in the metaclustering step by the ConsensusClusterPlus function. The delta area score (y-axis) indicates the relative increase in cluster stability obtained when clustering the 100 SOM codes generated by FlowSOM into k groups (x-axis).")
# 17
fig_nums(name = "tsne-plot-one-clustering2", caption = "t-SNE plot based on the arcsinh-transformed expression of the 10 lineage markers in the cells from the PBMC dataset. From each of the 16 samples, 2000 cells were randomly selected. Cells are colored according to the 12 cell populations obtained with FlowSOM after the metaclustering step with ConsensusClusterPlus.")
# 18
fig_nums(name = "plot-clustering-heatmap2", caption = "Heatmap of the median marker intensities of the 10 lineage markers in the 12 metaclusters obtained with FlowSOM after the metaclustering step with ConsensusClusterPlus (PBMC data). The heat represents the median of arcsinh and 0-1 transformed marker expression. The dendrogram on the left represents the hierarchical similarity between the 12 metaclusters (metric: Euclidean distance; linkage: average). Values in the brackets indicate the relative size of a given cluster.")
# 19
fig_nums(name = "tsne-plot-one-clustering2m", caption = "t-SNE plot based on the arcsinh-transformed expression of the 10 lineage markers in the cells from the PBMC dataset. From each of the 16 samples, 2000 cells were randomly selected. Cells are colored according to the 12 metaclusters obtained with FlowSOM after the metaclustering step with ConsensusClusterPlus.")
# 20 
fig_nums(name = "plot-clustering-heatmap2-merging", caption = paste0("Heatmap as in Figure 18", ifelse(on.bioc, fig_nums("plot-clustering-heatmap2", display = "num"), "\\@ref(fig:plot-clustering-heatmap2)"), " with color bars on the left indicating how the 12 metaclusters obtained with FlowSOM are merged into the 8 PBMC populations."))
# 21
fig_nums(name = "plot-clustering-heatmap2m", caption = "Heatmap of the median marker intensities of the 10 lineage markers in the 8 PBMC cell populations obtained by manual merging of the 12 metaclusters generated by FlowSOM. The heat represents the median of arcsinh and 0-1 transformed marker expression. Values in the brackets indicate the relative size of each of the cell populations across all the samples.")
# 22
fig_nums(name = "tsne-plot-one-clustering3", caption = "t-SNE plot with cells colored according to (A) the expert merging of 12 metaclusters obtained with FlowSOM into 8 PBMC populations; and (B) the 8 automatically detected with FlowSOM metaclusters.")
# 23
fig_nums(name = "plot-clustering-heatmap3", caption = "Heatmap of the median marker intensities of the 10 lineage markers in the 8 metaclusters obtained with FlowSOM after the metaclustering step with ConsensusClusterPlus (PBMC data). The heat represents the median of arcsinh and 0-1 transformed marker expression. The dendrogram on the left represents the hierarchical similarity between the 8 metaclusters (metric: Euclidean distance; linkage: average). Values in the brackets indicate the relative size of a given cluster.")
# 24
...
# 29
fig_nums(name = "diff-expr1-plot-median-expr", caption = "Median (arcsinh-transformed) expression of 14 signaling markers calculated from all the cells in a given sample in the PBMC dataset. Different colors are used for the two conditions unstimulated (Ref) and stimulated with BCR/FcR-XL (BCRXL). Values for each patient are indicated with different shape.")
# 30
fig_nums(name = "diff-expr1-plot-heatmap-with-significant-markers", caption = "Normalized expression for signaling markers in the PBMC dataset that are significantly differentially expressed between BCR/FcR-XL stimulated and unstimulated condition.")
...
cat("***Note:*** *to cite this article, please refer to
  https://f1000research.com/articles/6-748/v1 for instructions.*")
```
...
Another spot check is the number of cells per sample (see Figure `r ifelse(on.bioc, fig_nums("plot-number-of-cells", display = "num"), "\\@ref(fig:plot-number-of-cells)")`). This plot can be used as a guide together with other readouts to identify samples where not enough cells were assayed.
<!-- [CARSTEN] What is the minimum amount of cells recommended for analysis???? What is the maximum??? -->


```{r plot-number-of-cells, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("plot-number-of-cells"))}
...
```{r plot-dendogram, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("plot-dendogram"))}
library(RColorBrewer)
library(pheatmap)
...
```{r nrs, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("nrs"))}
...
```{r plot-clustering-distribution1, fig.height = 8, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("plot-clustering-distribution1"))}
...
plot_clustering_distr_wrapper <- function(expr, cell_clustering){
  ...
  ggplot() +
    geom_joy(data = ggd_plot, aes(x = expression, y = cluster, 
...
```{r plot-som-codes1-pca, fig.height = 8, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("plot-som-codes1-pca"))}
## Plot PCA on codes
...
```{r consensus-plot, out.width = '80%', fig.align='center', fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("consensus-plot"))}
...
In the t-SNE plot (see Figure `r ifelse(on.bioc, fig_nums("tsne-plot-one-clustering2", display = "num"), "\\@ref(fig:tsne-plot-one-clustering2)")`), we can see that many small clusters obtained when stratifying data into 20 groups are now merged together, which should simplify the new cluster annotation.  

```{r tsne-plot-one-clustering2, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("tsne-plot-one-clustering2"))}
...
```{r plot-clustering-heatmap2, fig.height = 3, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("plot-clustering-heatmap2"))}
plot_clustering_heatmap_wrapper(expr = expr[, lineage_markers_ord],
  expr01 = expr01[, lineage_markers_ord], cell_clustering = cell_clustering2,
  color_clusters = color_clusters)
```

Over-clustering into as few as 12 groups still allows us to identify the same 8 cell populations as when merging 20 clusters (see Figures `r ifelse(on.bioc, fig_nums("plot-clustering-heatmap2", display = "num"), "\\@ref(fig:plot-clustering-heatmap2)")`, `r ifelse(on.bioc, fig_nums("tsne-plot-one-clustering2m", display = "num"), "\\@ref(fig:tsne-plot-one-clustering2m)")`, `r ifelse(on.bioc, fig_nums("plot-clustering-heatmap2-merging", display = "num"), "\\@ref(fig:plot-clustering-heatmap2-merging)")`, `r ifelse(on.bioc, fig_nums("plot-clustering-heatmap2m", display = "num"), "\\@ref(fig:plot-clustering-heatmap2m)")`), but it is simpler to define since fewer profiles need to be manually scanned.  The expert-based merging is saved in the PBMC8_cluster_merging2.xlsx file on the [Robinson Lab server](http://imlspenticton.uzh.ch/robinson_lab/cytofWorkflow).
...
```{r tsne-plot-one-clustering2m, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("tsne-plot-one-clustering2m"))}
...
```{r plot-clustering-heatmap2-merging, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("plot-clustering-heatmap2-merging"))}
...
```{r plot-clustering-heatmap2m, fig.height = 3, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("plot-clustering-heatmap2m"))}
plot_clustering_heatmap_wrapper(expr = expr[, lineage_markers_ord],
...
In the t-SNE map (see Figure `r ifelse(on.bioc, fig_nums("tsne-plot-one-clustering3", display = "num"), "\\@ref(fig:tsne-plot-one-clustering3)")`), we can see that part of the new cell populations (cluster 7, 1 and 4, 2, 5 and 8) overlap substantially with populations obtained by the means of manual merging (CD4 T-cells, B-cells, surface-, monocytes and DC).  However, cells that belong to clusters 3 and 6 are subdivided in a different manner according to the manual merging.  Cluster 3 consists of CD8 T-cells and NK cells, and the latter cannot be identified anymore based on the heatmap corresponding to clustering into 8 groups (see Figure `r ifelse(on.bioc, fig_nums("plot-clustering-heatmap3", display = "num"), "\\@ref(fig:plot-clustering-heatmap3)")`). 

```{r tsne-plot-one-clustering3, fig.height = 8, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("tsne-plot-one-clustering3"))}
...
```{r plot-clustering-heatmap3, fig.height = 3, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("plot-clustering-heatmap3"))}
plot_clustering_heatmap_wrapper(expr = expr[, lineage_markers_ord],
...
The analysis of *overall* expression is analogous to the previous section, except that median marker expression is aggregated from all the cells in a given sample, Figure `r ifelse(on.bioc, fig_nums("diff-expr1-plot-median-expr", display = "num"), "\\@ref(fig:diff-expr1-plot-median-expr)")`.


```{r diff-expr1-plot-median-expr, fig.height = 8, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("diff-expr1-plot-median-expr"))}
...
As before, we create an output table with the median marker expression calculated in each sample and the p-values, and we plot a heatmap with the significant markers sorted by their statistical significance (Figure `r ifelse(on.bioc, fig_nums("diff-expr1-plot-heatmap-with-significant-markers", display = "num"), "\\@ref(fig:diff-expr1-plot-heatmap-with-significant-markers)")`).
...
```{r diff-expr1-plot-heatmap-with-significant-markers, fig.height = 3, fig.cap = gsub(ifelse(on.bioc, "", "Figure  [0-9]{1,2}: "), "", fig_nums("diff-expr1-plot-heatmap-with-significant-markers"))}
...
While we have a good understanding of how computational algorithms recapitulate manual gating in high dimensions [@Weber2016], one of the open areas of research remains how to best cluster *across* samples.    

[- A recent approach, called PAC-MAN [@Li116566], uses a combination of high dimensional density estimation, hierarchical clustering and network inference and comparison to extract clusters across samples, with a possibility to handle batch effects. In our approach, we aggregated all cells together before clustering.  An alternative would be to cluster within each sample and then aggregate a collection of metaclusters across samples [@Pyne2009]. Further research is required to better understand these effects, especially when batch effects are present.]
...
Additional challenges may arise when combining data from different instrument acquisitions and additional preprocessing treatments may need to be applied.  Despite adjustments through bead-based normalization [@Finck2013], the observed marker expression may be affected by the varying efficiency of antibody binding in each batch and by the ion detection sensitivity after machine calibration.  Beyond normalization, other strategies have been proposed, such as equalizing the dynamic range between batches for each marker or the use of warping functions to eliminate non-linear distortions (see `r Biocpkg("cydar")` vignette). 

[+ Alternatively, one could consider batch-wise clustering of samples. On the other hand, to be able to use those results, one still needs to match cell populations across batches. The matching could be done manually, or with the automated approaches developed for flow cytometry [@Pyne2009].]

However, a comprehensive evaluation of these approaches and their effect on downstream analyses is still missing, especially when batch effects are present.  Overall, we expect that as a general rule, including batch parameters (or other covariates) in the linear modeling largely mitigates the problem.
...
[+ Another set of methods (`r Biocpkg("MIMOSA")` and `r Biocpkg("COMPASS")`), based on Bayesian hierarhical framework, was proposed in the vaccine development field, where the antigen-specific T-cell response to stimulation for each subject is modelled using mixtures of beta-binomial or Dirichlet-multinomial distributions [@Finak2014; @Lin2015b], which similarly to the mixed models applied for differential abundance in this workflow aim to handle over-dispersion due to subject-to-subject variability.]
