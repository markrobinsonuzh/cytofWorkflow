...
# spot check that metadata and flowSet are in same order
all( rownames(phenoData(fs)) == md$file_name )
```
...
<!--```{r eval = FALSE}
library(flowCore)
fs <- read.flowSet(files = fcs_file_names, path = path_to_fcs_files,
...
Usually, the raw marker intensities read by a cytometer have strongly skewed distributions with varying ranges of expression, thus making it difficult to  distinguish between the negative and positive cell populations.  It is common practice to transform CyTOF marker intensities using, for example, arcsinh (hyperbolic inverse sine) with cofactor 5 [@Bendall687 Figure S2; @Bruggner2014] to make the distributions more symmetric and to map them to a comparable range of expression, which is important for clustering. A cofactor of 150 has been promoted for flow cytometry, but users are free to implement alternative transformations, some of which are available from the `transform` function of the `r Biocpkg("flowCore")` package. 

By default, the `daFrame` constructor will [TODO: finish this]

<!-- [X]
In the following step, we include only those channels that correspond to the lineage and functional markers.  We also rename the columns in the `flowSet` to the antigen names from `panel$desc`. -->
...
As a final note, the `daFrame` constructor requires the input `flowSet`'s or .fcs files' names to match with the filenames listed in the `md$file` column. In our case, there are no filenames specified in `fs` and we add them here manually.

```{r}
# specify levels for conditions
md$condition <- factor(md$condition, levels = c("Ref", "BCRXL"))
#md$sample_id <- md$sample_id[with(md, order(condition))]

# add filenames to description slot [TODO: is this necessary?]
all(is.null(unlist(keyword(fs, "FILENAME"))))
for (i in seq_along(fs))
    description(fs[[i]])$FILENAME <- identifier(fs[[i]])

# construct daFrame
daf <- daFrame(fs, panel, md, 
               cols_to_use = panel$fcs_colname)
```
...
Another spot check is the number of cells per sample (see Figure \@ref(fig:plot-number-of-cells)). This plot can be used as a guide together with other readouts to identify samples where not enough cells were assayed. The number of cells measured in each sample is also stored under `n_cells` in the `daFrame`'s `metadata`.
...
t-SNE requires significant computational time to process the data even for tens of thousands of cells. CyTOF datasets are usually much larger and thus to keep running times reasonable, one may use a subset of cells; for example, here we use 2000 cells from each sample.  The t-SNE map below is colored according to the expression level of the CD4 marker, highlighting that the CD4+ T-cells are placed at the top of the plot (see Figure \@ref(fig:tsne-plot-one-expr-CD4)). In this way, one can use a collection of markers to highlight where cell types of interest are located on the *map*.

Instead of t-SNE, one could also use other dimension reduction techniques, such as PCA, diffusion maps [TODO: add ref to destiny?], SIMLR [@Wang2017], isomaps or UMAP [TODO: add ref to UMAP].  To speed up the t-SNE analysis, one could use a multicore version that is available via the `r Githubpkg("RGLab/Rtsne.multicore")` or `r Githubpkg("DmitryUlyanov/Multicore-TSNE")` packages.  Alternative algorithms, such as `largeVis` [@Tang2016] (available via the `r Githubpkg("elbamos/largeVis")` package) or Hierarchical Stochastic Neighbor Embedding (HSNE; [TODO: cite https://www.nature.com/articles/s41467-017-01689-9]), can be used for dimensionality reduction of very large datasets without downsampling.  Alternatively, the dimensionality reduction can be performed on the *codes* of the SOM, at a resolution (size of the SOM) specified by the user (Figure \@ref(fig:plot-som-codes1-pca)).

```{r run-tsne}
daf <- tSNE(daf, n = 2000, seed = 1234)
```
...
## Facet per condition
plotSNE(daf, color_by = "meta20", facet = "condition")
```
...
In our setup, over-clustering is also useful when the interest is identifying the "natural" number of clusters present in the data. In addition to heatmaps and t-SNE plots, one could investigate the delta area plot from the `r Biocpkg("ConsensusClusterPlus")` package and the hierarchical clustering dendrogram of the over-clustered subpopulations, as shown in Figure \@ref(fig:plot-clustering-heatmap1m-merging) and \@ref(fig:consensus-plot), respectively.
...
merging_table1$new_cluster <- factor(merging_table1$new_cluster, 
    levels = c("B-cells IgM+", "B-cells IgM-", "CD4 T-cells", "CD8 T-cells",
               "DC", "NK cells", "monocytes", "surface-"))

daf <- mergeClusters(daf, k = "meta20", table = merging_table1, id = "merging1")
```

We can view the t-SNE plot with the new annotated cell populations by specifying `color_by = "merging1"` (see Figure \@ref(fig:tsne-plot-one-clustering1m)).
...
As an example, we can view the metaclustering to 12 groups. In the t-SNE plot (see Figure \@ref(fig:tsne-plot-one-clustering2)), we can see that many small clusters obtained when stratifying data into 20 groups are now merged together, which should simplify the new cluster annotation.
...
plotClusterHeatmap(daf, k = "meta12", draw_freqs = TRUE)
```
...
```{r cluster-merging2}
merging_table2 <- "PBMC8_cluster_merging2.xlsx"
download.file(sprintf("%s/%s", url, merging_table2), 
...
daf <- mergeClusters(daf, k = "meta12", table = merging_table2, id = "merging2")
```
...
plotClusterHeatmap(daf, k = "meta12", m = "merging2")
```
...
plotClusterHeatmap(daf, k = "merging2", draw_freqs = TRUE)
```
...
## Get cluster ids for each cell
table(
    algorithm = cluster_codes(daf)[cluster_ids(daf), "meta12"],
    manual = cluster_codes(daf)[cluster_ids(daf), "merging2"])
```
...
p1 <- plotSNE(daf, color_by = "merging2")
p2 <- plotSNE(daf, color_by = "meta8")
...
plotClusterHeatmap(daf, k = "meta8", draw_freqs = TRUE)
```
...
An output table containing the observed cell population proportions in each sample and p-values can be assembled (and optionally written to a file) as follows. 

```{r diff-freqs-fit-model-output}
counts <- assay(da_res2$d_counts)
props <- t(t(counts) / colSums(counts)) * 100
props <- props[, order(ei$condition)] # reorder samples
da_output2 <- data.frame(row.names = NULL,
    cluster_id = rownames(props), props,
    rowData(da_res2$res)[, c("p_val", "p_adj")])
...
plotDiffHeatmap(daf, da_res2, hm1 = FALSE, normalize = TRUE)
```
...
It is helpful to plot the median expression of all markers in each cluster for each sample colored by condition, to get a rough image of how strong the differences might be (see Figure \@ref(fig:diff-expr2-plot-median-expr)).  We do this by combining boxplots and jitter.
...
```{r ds-output}
meds <- assays(ds_res2$d_medians)   # get median marker expressions
meds <- meds[state_markers(daf)]    # subset state markers
meds <- do.call(rbind, meds)        # concatenate all markers

ds_output2 <- data.frame(
    rowData(ds_res2$res), 
    row.names = NULL, meds)
print(head(ds_output2), digits = 2)
```
...
daf <- mergeClusters(daf, k = "meta20", id = "merging_all",
    table = data.frame(old_cluster = seq_len(20), new_cluster = 1))
```
...
```{r ds-output-overall}
meds <- assays(ds_res4$d_medians)   # get median marker expressions
meds <- meds[state_markers(daf)]    # subset state markers
meds <- do.call(rbind, meds)        # concatenate all markers

ds_output4 <- data.frame(
    rowData(ds_res4$res)[, -1], 
    meds, row.names = NULL)
print(head(ds_output4), digits = 2)
```
...
plotDiffHeatmap(daf, ds_res4, all = TRUE, normalize = TRUE, hm1 = FALSE)
```
